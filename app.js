...
document.getElementById('pdfInput').addEventListener('change', async (event) => {
  ...
  try {
    ...
    const geraetMatch = text.match(/Gerätenummer[:\s]*([\w-]+)/i);
    ...
    const geraet = geraetMatch ? geraetMatch[1] : "–";

    // Adresse geokodieren via OpenStreetMap
    ...
    console.error(error);
    alert("Fehler beim Auslesen der PDF. Siehe Konsole für Details.");
  }
});
